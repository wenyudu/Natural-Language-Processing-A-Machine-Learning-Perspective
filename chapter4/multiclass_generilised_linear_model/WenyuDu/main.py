...
This is a simple implementation of the multi-class generilised linear classifiers that optimised by SGD.
...
class Multiclass_SVM():
    def __init__(self,data,split=1.0):
        ...
        self.epoch_number = 20
        self.lr = 0.3 # learning rate
        self.C = 0.0000001 # l2 reguliser lambda
        self.model = nn.Linear(self.voc_size * self.class_size, 1) # w, b in pytorch
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_number):
            ...
            for i in range(self.train_size):
                ...
                ###### loss function ######
                if max_output_idx != y: # condition of updating parameters of w and b
                    loss = torch.clamp(1-outputs[y]+outputs[max_output_idx], min=0) # loss function
                    sum_loss += self._optimizer_step(optimizer, loss)  # sum of loss in one epoch
                #############################
                ...
                with torch.no_grad():
                    self.model.weight -= self.lr * self.C * weight
            # self.test()
            print("Epoch: {:4d}\tloss: {}".format(epoch, sum_loss / self.train_size))
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_number):
            for i in range(self.train_size):
                ...
                # calculate outputs and probabilities for all classes c
                outputs = []
                for j in range(self.class_size):
                    outputs.append(self.model(torch.FloatTensor(multiclass_X[j][i])).squeeze()) # wv+b for all class in pytorch
                probabilities = torch.softmax(torch.stack(outputs),dim=0) # get the probability for each class

                ###### loss function ######
                updated = 0.0
                with torch.no_grad():
                    for j in range(self.class_size):
                        updated -= (multiclass_X[y][i] - multiclass_X[j][i]) * probabilities[j] # the update rule
                    self.model.weight -= self.lr * updated
                #############################
...
if __name__ == '__main__':
    ...
    # classifier = Multiclass_SVM(data,0.2)
