...
This is a simple implementation of the binary generilised linear classifiers that optimised by SGD.
...
class Binary_Perceptron():
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_number):
            ...
            for i in range(self.train_size):
                ...
                if Y[i] * output > 0: # condition of updating parameters of w and b
                    continue
                loss = torch.clamp(-Y[i] * output, min=0) # loss function
    ...
    def test(self):
        ...
        for i,s in enumerate(self.data.test["sents"]):
            output = "Positive" if self.model(X[i]).squeeze().data > 0 else "Negative" # dot product in pytorch
            print('*' * 20)
            print(' '.join(s),": ",output)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_number):
            ...
            for i in range(self.train_size):
                ...
                ###### loss function ######
                output = self.model(X[i]).squeeze() # wv+b in pytorch
                if Y[i] * output < 1: # condition of updating parameters of w and b
                    loss = torch.clamp(1-Y[i] * output, min=0) # loss function
                    sum_loss += self._optimizer_step(optimizer,loss) # sum of loss in one epoch
                #############################
...
if __name__ == '__main__':
    ...
    classifier = Binary_Perceptron(data)
    # classifier = Binary_SVM(data)
